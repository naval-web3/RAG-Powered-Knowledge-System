...
The question here is not whether retrieval-augmented generation can be built — it plainly can — but whether it can be built on one laptop, offline, by one person, in a semester.
...
**Is an embedded vector database sufficient?** ChromaDB in persistent mode requires no server process, no separate installation and no port. It stores its index on disk beside the application. For a single-machine deployment holding thousands rather than millions of chunks this is not merely sufficient, it is preferable, because it removes an entire class of deployment failure.
...
The diagrams in this section use the Yourdon–DeMarco convention throughout: a **circle** is a process that transforms data, a **rectangle** is an external entity outside the system's control, a pair of **open-ended parallel lines** is a data store, and an **arrow** is a flow of data labelled with what it carries. A **dashed arrow** marks a flow that occurs only under a stated condition. Processes are numbered so that process 3.1 is a component of process 3.0, and every flow crossing the boundary of an exploded process appears on both the parent and the child diagram.

One remark on the drawings is worth making. The data flow diagrams in the approved synopsis showed the vector store as an external entity inside a diamond. That is wrong twice over — the vector store is a data store belonging to the system rather than an entity outside it, and a diamond is entity-relationship notation for a relationship, not data flow notation for anything. The diagrams here are drawn afresh and the vector store appears as data store D4 throughout.

### Context Diagram

The context diagram fixes the system's boundary. It has exactly one process — the system as a whole — and shows only what crosses that boundary.
...
Only on the path through 4.5 and 4.6 does a prompt reach the model, and by then the prompt contains the grounding rules, the numbered passages, the project's standing instructions and the recent history — everything the answer is permitted to draw on. Figure 2.7 is the exploded process.
...
The **language model provider** appears as a supporting actor attached only to the generation use case, which records that the system depends on it for that one thing and for nothing else. The model is drawn in Figure 2.9.
